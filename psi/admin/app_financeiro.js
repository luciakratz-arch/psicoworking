...
function PacoteForm({ usuario, pacientes, pacote, config, parceiras, aoFechar }) {
  ...
  async function executarSalvar(tipoVenda) {
    ...
    try {
      ...
      const refsSessoes = datas.map((data, i) => ({ ref: db.collection("clinica_sessoes").doc(), data, numSessao: i + 1 }));
      const batch = db.batch();
      refsSessoes.forEach(({ ref, data, numSessao }) => {
        batch.set(ref, {
          psi_id: usuario.psiId,
          pacienteId: form.pacienteId, pacienteNome: pac?.nome || "",
          data, hora: form.horario, duracao: "50", tipo: "Psicoterapia",
          status: "agendado", numSessao, pacoteId: pacRef.id,
      ...
      try {
        ...
        for (const { ref, data, numSessao } of refsSessoes) {
          ...
          if (resultado?.data?.eventoId) {
            await ref.update({ eventoGoogleId: resultado.data.eventoId });
          }
        }
      } catch (eAgenda) {
